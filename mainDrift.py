...
def main():
    ...
    subplot.plot(y_cent_nan, linewidth=0.5, color="k", label="frequency")
    ...
    subplot.plot(histogram_smooth, color="b", label="smoothed histogram")
    ...
    fig, subplots = plt.subplot_mosaic([["audio", "audio", "audio", "audio", "legend"],
                                        ["cluster", "cluster", "cluster", "cluster", "legend"],
                                        ["cluster", "cluster", "cluster", "cluster", "legend"],
                                        ["cluster", "cluster", "cluster", "cluster", "legend"],
                                        ["cluster", "cluster", "cluster", "cluster", "legend"]],
                                       figsize=(20, 10))
    subplots["cluster"].sharex(subplots["audio"])
    subplots["legend"].tick_params(bottom=False, labelbottom=False,
                                   left=False, labelleft=False)
    subplots["legend"].spines['left'].set_visible(False)
    subplots["legend"].spines['right'].set_visible(False)
    subplots["legend"].spines['bottom'].set_visible(False)
    subplots["legend"].spines['top'].set_visible(False)
    ...
    for p in range(n_clusters_):
        ...
        subplots["cluster"].plot(xrange, yrange / 10, linestyle="dashed", color="k", linewidth=0.6,
    ...
    for idx, segment in np.ndenumerate(seg):
        ...
        subplots["cluster"].scatter(scatter_x, scatter_y, s=0.005, c=colors_)
    ...
    subplots["legend"].legend(handles=legend_handles, labels=legend_labels)
    for name in ["audio", "cluster"]:
        subplots[name].legend().remove()
    fig.show()
